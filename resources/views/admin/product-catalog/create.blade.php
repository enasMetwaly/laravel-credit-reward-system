                ...
                <form action="{{ route('admin.product-catalog.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="is_redeemable" id="is_redeemable" class="form-control" required>
                            <option value="0" {{ old('is_redeemable') == 0 ? 'selected' : '' }}>No</option>
                            <option value="1" {{ old('is_redeemable') == 1 ? 'selected' : '' }}>Yes</option>
                        </select>
                    </div>
                    <div class="form-group" id="points_field" style="{{ old('is_redeemable') == 1 ? '' : 'display:none;' }}">
                        <label for="points_required">Points Required</label>
                        <input type="number" name="points_required" id="points_required" class="form-control" value="{{ old('points_required') }}" min="1">
                    </div>
